...
def get_version(*args, **kwargs):
    ...
    * *alpha* means this version is either an alpha release, or a development version.
    ...
    :py:data:`xobox.VERSION` is used instead.
    ...
    :param kwargs: keyword arguments, of which only the value of ``version`` (if present) will be taken as
                   version quintuple
    :returns: `PEP 440`_ compliant version string
    ...
    if version[3] == 'alpha' and version[4] == 0:
        git_revision = get_git_changeset()
        if git_revision:
...
def get_git_changeset(path=None):
    ...
    of `PEP 440`_, the UTC timestamp in YYYYMMDDHHMMSS format is used
    ...
    :param str path: Path to the `Git`_ repository to detect the latest commit timestamp from.
                     If not indicated, the parent path of the xobox package is used.
    ...
    """
    if path is None:
        path = os.path.normpath(os.path.join(xobox.__path__[0], ".."))

    # run `git show` in xobox' root directory and grab its output from stdout
    ...
    return '{0:>s}'.format(timestamp.strftime('%Y%m%d%H%M%S'))
