...
class Singleton:
    ...
    * To get the singleton instance, the :py:meth:`~xobox.utils.singleton.Singleton.get_instance`
      method has to be used. Trying to use ``__call__`` will result in a :py:exc:`TypeError` being raised.
    * The actual instance will not be created before :py:meth:`~xobox.utils.singleton.Singleton.get_instance`
